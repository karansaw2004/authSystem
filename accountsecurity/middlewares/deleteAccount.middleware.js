...
export function deleteAccountMiddleware(req, reply, done) {
    try {
        ...
        if (!sanitizedData.deviceFingerPrint) {
            return reply.send(new ApiError("Device Finger Print is required", 400));
        }
        req.body = sanitizedData;
        return done();
    } catch (error) {
        console.log("error in the middleware function of the deleteAccount route", error.message);
