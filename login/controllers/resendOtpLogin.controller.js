...
export async function handleResendOtpLogin(req,reply) {
    try {
        const {mail,deviceFingerPrint} = req.body;
        const userId = securityManager.createUserId(mail);
        let data = await redis.get(`twoFactorAuthentication:${userId}`);
        if (!data) {
        ...
        if (!verifyDevice.success) {
            return reply.send(new ApiError("Device not recognized. Please use the device used during login",403));
        };
        const newOtp = generateOtp().toString();
        data.otp = newOtp;
        const sendMail = await SendOtp(mail,"login",newOtp);
        if (!sendMail) {
            return reply.send(new ApiError("Failed to send OTP. Please try again later",500));  
        };
        await redis.set(`twoFactorAuthentication:${userId}`,JSON.stringify(data),300);
