...
export async function handleResendOtpRegister(req,reply) {
    try {
        ...
        if (!verifyDevice.success) {
            return reply.send(new ApiError("Device not recognized. Please use the device used during registration",403));
        };
        const newOtp = generateOtp().toString();
        data.otp = newOtp;
        const sendMail = await SendOtp(mail,"register",newOtp);
        if (!sendMail) {
